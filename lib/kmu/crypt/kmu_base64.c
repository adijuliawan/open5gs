...
int kmu_base64_decode(char *bufplain, const char *bufcoded)
{
    ...
    len = kmu_base64_decode_binary((unsigned char *) bufplain, bufcoded);
    bufplain[len] = '\0';
    return len;
}

/* This is the same as kmu_base64_decode() except:
 * - no \0 is appended
 */
int kmu_base64_decode_binary(unsigned char *bufplain, const char *bufcoded)
{
    int nbytesdecoded;
    register const unsigned char *bufin;
...
int kmu_base64_encode(char *encoded, const char *string, int len)
{
    return kmu_base64_encode_binary(

...
void ogs_kdf_prf_prime(
    uint8_t *ik_prime, uint8_t *ck_prime, const char *supi,
    ...
    ogs_debug("[EAP_AKA_PRIME][PRF'] SUPI : [%s], length SUPI : [%ld]", val,strlen(val));
    memset(param, 0, sizeof(param));
    param[0].buf = (uint8_t *) "EAP-AKA'";
    param[0].len = strlen("EAP-AKA'");
    ogs_debug("[EAP_AKA_PRIME][PRF'] EAP-AKA' length EAP-AKA; : [%ld]", strlen("EAP-AKA'"));
    param[1].buf = (const uint8_t*) val;
    
    ogs_assert(param[1].buf);
    param[1].len = strlen(val);
    ...
    for (i=0; i< MAX_NUM_OF_KDF_PARAM && param[i].buf && param[i].len; i++){
        input_len+= (param[i].len);
    }

    input = ogs_calloc(1,input_len);
    ogs_assert(input);

    /* Copy buffer from param */
    input_len = 0;
    for (i = 0; i < MAX_NUM_OF_KDF_PARAM && param[i].buf && param[i].len; i++) {
        memcpy(&input[input_len], param[i].buf, param[i].len);
        input_len += param[i].len;
    }
    /*
    for (i = 0; i < input_len; i++) {
        ogs_debug("[EAP_AKA_PRIME][PRF'] input : [%02x]", input[i]);
    }
    */
    char initial_input_str[2*input_len+1]; // 32 bytes * 2 hex chars + 1 null terminator

    for (i = 0; i < input_len; i++) {
        // Write exactly 2 characters into each pair slot
        snprintf(&initial_input_str[i * 2], 3, "%02x", input[i]);
        // 3 = 2 digits + 1 null-terminator for that slot
    }
    initial_input_str[2*input_len] = '\0';

    char key_str[2*32+1]; // 32 bytes * 2 hex chars + 1 null terminator
    ...
    ogs_debug("[EAP_AKA_PRIME][PRF'] input_length : [%d]", input_len);
    ogs_debug("[EAP_AKA_PRIME][PRF'] input : [%s]", initial_input_str);
    ogs_debug("[EAP_AKA_PRIME][PRF'] key_length : [%d]", 32);
    ogs_debug("[EAP_AKA_PRIME][PRF'] key : [%s]", key_str);

    int round = 208 / 32 + 1;  
    ogs_debug("[EAP_AKA_PRIME][PRF'] round : [%d]", round);
    ...
    for (i = 0; i < round ; i++){
        ...
        s[pos++] = (uint8_t)(i + 1);
        ogs_debug("[EAP_AKA_PRIME][PRF'] Round[%d] Input_length[%d] ",i+1,s_len);
        ogs_debug("[EAP_AKA_PRIME][PRF'] Round[%d] pos_length[%d] ",i+1,pos);

        int k;
        /*
        for (k = 0; k < pos; k++) {
            ogs_debug("[EAP_AKA_PRIME][PRF'] Round[%d] Input[%02x] ",i,s[k]);
        }
        */
        ...
        input_str[2*pos] = '\0';
        ogs_debug("[EAP_AKA_PRIME][PRF'] Round[%d] Input[%s] ",i+1,input_str);
        ...
        //ogs_hmac_sha256(key, OGS_KEY_LEN*2 , s, pos,  T[i], OGS_SHA256_DIGEST_SIZE);
        ogs_hmac_sha256(key, OGS_KEY_LEN*2 , s, s_len,  T[i], 32);
    ...
    for (i = 0; i < round; i++) {
        memcpy(output + i * 32, T[i], 32);
        free(T[i]);
    }

    memcpy(k_encr, output, OGS_KEY_LEN);
    memcpy(k_aut, output+OGS_KEY_LEN, OGS_KEY_LEN*2);
    memcpy(k_re, output+OGS_KEY_LEN*3, OGS_KEY_LEN*2);
    memcpy(msk, output+OGS_KEY_LEN*5, OGS_KEY_LEN*4);
    memcpy(emsk, output+OGS_KEY_LEN*9, OGS_KEY_LEN*4);



    /*
    // Allocate result buffer
    //uint8_t **T = malloc(round * OGS_SHA256_DIGEST_SIZE);
    uint8_t **T = ogs_calloc(round, OGS_SHA256_DIGEST_SIZE);
    
    //parameter
    // input, input_len, key, key_len, output_len = 208

    for ( i = 0; i < round; i++) {
        size_t s_len = (i == 0 ? input_len : OGS_SHA256_DIGEST_SIZE) + input_len + 1;
        uint8_t *s = malloc(s_len);
        ...
        if (i == 0) {
            ...
            memcpy(s + pos, T[i - 1], OGS_SHA256_DIGEST_SIZE);
        ...
        //HmacSha256(T[i], s, pos, key, key_len);
        //ogs_hmac_sha256(key, OGS_KEY_LEN, s, s_len, result + i*OGS_SHA256_DIGEST_SIZE, OGS_SHA256_DIGEST_SIZE);
        ogs_hmac_sha256(key, OGS_KEY_LEN, input, input_len, T[i], OGS_SHA256_DIGEST_SIZE);
        free(s);
    }

    size_t total_len = round * OGS_SHA256_DIGEST_SIZE;
    uint8_t *output[total_len];

    for ( i = 0; i < round; i++) {
        memcpy(*output + i * OGS_SHA256_DIGEST_SIZE, T[i], OGS_SHA256_DIGEST_SIZE);
        //free(T[i]);
    }
    */

    //*output_actual_len = total_len;
    ...
    //     // Construct the input to HMAC
    //     // s = (i==0 ? input : T[i-1]) || input || counter
    //     size_t s_len = (i == 0 ? 0 : OGS_SHA256_DIGEST_SIZE) + input_len + 1;
    //     uint8_t *s = malloc(s_len);

    //     size_t offset = 0;
    //     if (i > 0) {
    //         memcpy(s, result + (i - 1) * OGS_SHA256_DIGEST_SIZE, OGS_SHA256_DIGEST_SIZE);
    //         offset += OGS_SHA256_DIGEST_SIZE;
    //     }

    //     memcpy(s + offset, input, input_len);
    //     offset += input_len;
